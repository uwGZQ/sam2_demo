    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        try:
            ...
            if not os.path.exists(in_path) or os.path.getsize(in_path) == 0:
                raise Exception("Input file is empty or does not exist")

            video_metadata = get_video_metadata(in_path)
            print(f"Input video metadata: {video_metadata}")
            ...
            print(f"Processing video with start_time={start_time_sec}, duration={duration_time_sec}")
            ...
            if not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
                raise Exception("Transcoded output file is empty or does not exist")

            out_video_metadata = get_video_metadata(out_path)
            print(f"Output video metadata: {out_video_metadata}")
