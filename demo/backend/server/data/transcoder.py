...
def normalize_video(
    ...
    # 添加输入视频的时长检查
    if in_metadata.duration_sec is not None and in_metadata.duration_sec < seek_t:
        raise Exception(f"seek time {seek_t} is greater than video duration {in_metadata.duration_sec}")
    ...
    if not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
        raise Exception(f"FFmpeg produced empty or non-existent output file: {out_path}")
