...
const sharedStyles = stylex.create({
  ...
  activeContainer: {
    ...
    // Increase max height for active containers to accommodate attributes
    maxHeight: {
      default: '500px', // Increased from default
  ...
  rightColumn: {
    ...
    flexGrow: 1,
    alignItems: 'center',
    width: '100%', // Ensure full width for the attributes area
  },
});
...
type ToolbarObjectContainerProps = PropsWithChildren<{
  ...
  isActive: boolean;
  title: ReactNode; // Changed from string to ReactNode to support components
  subtitle: string;
  thumbnail: ReactNode;
  isMobile: boolean;
  onCancel?: () => void;
