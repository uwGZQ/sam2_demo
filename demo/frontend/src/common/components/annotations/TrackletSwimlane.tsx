...
import ObjectAttributesDisplay from '@/common/components/annotations/ObjectAttributesDisplay';
...
//   console.log("Masks Data:", masks); // 打印 mask 数据
  if (masks.length === 0) {
    return [];
  }
...
export default function TrackletSwimlane({tracklet, onSelectFrame}: Props) {
  ...
  const objectLabel = getObjectLabel(tracklet, customLabels);
            ...
            >
              {hoveredPoint === index && (
                <div {...stylex.props(styles.tooltip)}>
                  <div className="font-bold mb-1">{objectLabel} - Frame {index}</div>
                  <ObjectAttributesDisplay 
